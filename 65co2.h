...
struct SixtyFiveCeeOhTwo
{
	...
	typedef struct __registers
	{
		...
		inline unsigned char SRgetByte(bool SR_BREAK)
		{
	...
	void setPC(unsigned short adr)
	{
		r.PC = adr;
	}

	void push_cpu_interrupt(unsigned short PC, bool fromInt)
	{
		push_stack((unsigned char)((PC & 0xFF00) >> 8));
		push_stack((unsigned char)(PC & 0xFF));
	...
	void dumpCpuState()
	{
		...
		{
		case 0:
			printf("%02X       %s ", opcode, nmem[OPCODES[opcode].op]);
			break;
		case 1:
			printf("%02X %02X    %s ", opcode, zpage, nmem[OPCODES[opcode].op]);
			break;
		case 2:
			printf("%02X %02X %02X %s ", opcode, address & 0xFF, (address & 0xFF00) >> 8, nmem[OPCODES[opcode].op]);
			break;
		}
